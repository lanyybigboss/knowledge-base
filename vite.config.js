...
export default defineConfig({
  ...
  build: {
    outDir: 'dist',
    sourcemap: false,  // 生产环境关闭 sourcemap，减少打包体积和内存占用
    rollupOptions: {
      output: {
        // 手动分块，优化缓存和加载性能
        manualChunks: {
          ...
          'utils-vendor': ['fuse.js', 'dexie'],
          'ai-vendor': ['tesseract.js', 'pdfjs-dist'],
          'office-vendor': ['mammoth', 'xlsx']
        }
      }
    },
    chunkSizeWarningLimit: 1000  // 提高 chunk 大小警告阈值到 1MB
  },
  optimizeDeps: {
    include: ['react', 'react-dom', 'react-router-dom', 'lucide-react', 'fuse.js', 'dexie'],
    exclude: ['tesseract.js']  // tesseract.js 很大，动态导入
  }
})
