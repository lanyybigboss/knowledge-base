...
function _tryJsonRepair(text) {
  ...
  return { text: repaired, repaired: repaired !== text }
}
...
function normalizeResult(analysis) {
  ...
  return {
    ...
    summary: (analysis.summary || '').replace(/[\x00-\x1F\x7F]/g, ''),  // 去除控制字符
    detailedSummary: (analysis.detailedSummary || analysis.summary || '').replace(/[\x00-\x1F\x7F]/g, ''),
    keywords: (analysis.keywords || []).filter(Boolean).slice(0, 8),
    tags: tags.slice(0, 3),
    entities: analysis.entities || { people: [], organizations: [], locations: [], dates: [] },
    smartTitle: (analysis.smartTitle || '').substring(0, 20).replace(/[\x00-\x1F\x7F]/g, '')
  }
}
 ...
 * 
 * 降级策略（v1.5.1）：
 *   Ollama (qwen2.5:3b) 优先 → DeepSeek API 后备 → 返回降级结果(_fallback:true)
 */
...
文档内容：${(content || '').substring(0, 4000)}`
  ...
  try {
    ...
    // === 结果处理 ===
    if (analysis) {
      const normalized = normalizeResult(analysis)
  ...
  for (const item of files) {
    try {
      ...
      if (!content || content.trim().length < 10) {
        logger.warn(`文件 ${file.name} 内容太少，跳过 AI 分析`)
